...
def test_calculate_taxes_basic(list_float):
    assert calculate_taxes(list_float, 10.0) == [110.0, 165.0, 22.55, 643.5]
    assert calculate_taxes(list_float, 0.0) == [100.0, 150.0, 20.5, 585.0]
...
@pytest.mark.parametrize('prices, tax_rate, expected_result', [
    ([10.0, 15.0, 2.05, 58.5], 10.0, [11.0, 16.5, 2.255, 64.35]),
    ([], 5.0, []),
    ([100.0, 15.0, 50.5], 20.5, [120.5, 18.075, 60.8525])
])
